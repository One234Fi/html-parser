...
void file_read_tests() {
...

    {
        ...
        const ptrdiff_t expected_len = strlen(expected_result);

        input_system s = input_system_init("testdata/short.html", &temp);

        TEST(s.buffer.len == expected_len, "Short file length should be calculated correctly");
        TEST(strncmp(s.buffer.data, expected_result, expected_len) == 0, "Short file contents should be parsed correctly");
    }

    {
        ...
        const ptrdiff_t long_len = strlen(long_result);

        input_system s = input_system_init("testdata/archlinux.html", &temp);

        TEST(s.buffer.len == long_len, "Long file length should be calculated correctly");
        TEST(strncmp(s.buffer.data, long_result, long_len) == 0, "Long file contents should be parsed correctly");
    }
}
