...
void vector_tests() {
...

    {
        ...
        *push(&v, &temp) = 'a';
        TEST(*v.data == 'a', "a should be set");
        TEST(v.len == 1, "len should be 1");
        TEST(v.cap == 2, "cap should be 2");

        *push(&v, &temp) = 'b';
        TEST(v.data[1] == 'b', "b should be set");
        TEST(v.len == 2, "len should be 2");
        TEST(v.cap == 2, "cap should be 2");

        *push(&v, &temp) = 'c';
        TEST(v.data[2] == 'c', "c should be set");
        TEST(v.len == 3, "len should be 3");
        TEST(v.cap == 4, "cap should be 4");
    }

    {
        ...
        *push(&vi, &temp) = 1;
        ...
        TEST(*vi.data == 1, "1 should be set");
        TEST(vi.len == 1, "len should be 1");
        TEST(vi.cap == 2, "cap should be 2");

        *push(&vi, &temp) = 2;
        TEST(vi.data[1] == 2, "2 should be set");
        TEST(vi.len == 2, "len should be 2");
        TEST(vi.cap == 2, "cap should be 2");

        *push(&vi, &temp) = 3;
        TEST(vi.data[2] == 3, "3 should be set");
        TEST(vi.len == 3, "len should be 3");
        TEST(vi.cap == 4, "cap should be 4");
    }
}
...
void concat_tests() {
    ...
    ptrdiff_t expected_len = strlen(expected_str);
    ...
    for (uint32_t i = 0; i < expected_len; i++) {
        fprintf(stderr, "%c", sb.data[i]);
    }
    fprintf(stderr, "\n");

    TEST(strncmp(sb.data, expected_str, expected_len) == 0, "Expected output should match input strings");
    TEST(sb.len == expected_len, "Length should equal the sum of the input lengths");
}

void find_diff(string a, string b) {
    if (a.len != b.len) {
        fprintf(stdout, "lengths: a: %td, b: %td\n", a.len, b.len);
    }

    ptrdiff_t len = a.len < b.len ? a.len : b.len;
    for (int i = 0; i < len; i++) {
        if (a.data[i] != b.data[i]) {
            fprintf(stdout, "DIFF: %c, %c\n", a.data[i], b.data[i]);
        }
    }
}
...
void newline_normalization_tests() {
    ...
    char s[] = "This is a \r\nstring with\r\nnewlines";
    string raw_str = make_string(s);
    char* exp_str = "This is a \nstring with\nnewlines";
    normalize_newlines(&raw_str);
    TEST(strncmp(raw_str.data, exp_str, raw_str.len) == 0, "Carriage return should be replaced with newlines");


    char s2[] = "\r\nThis is a \nstring\r\nwith\nmixed newlines\n";
    string case2 = make_string(s2);
    char* res2 = "\nThis is a \nstring\nwith\nmixed newlines\n";
    normalize_newlines(&case2);
    TEST(strncmp(case2.data, res2, case2.len) == 0, "Carriage return should be replaced with newlines");


    char s3[] = "\r\n\r\nStart with two newlines";
    string edgecase = make_string(s3);
    char* edgeres = "\n\nStart with two newlines";
    normalize_newlines(&edgecase);
    TEST(strncmp(edgecase.data, edgeres, edgecase.len) == 0, "Carriage return should be replaced with newlines");


    char s4 [] = "End with two newlines\r\n\r\n";
    string endlines = make_string(s4);
    char* endlinesres = "End with two newlines\n\n";
    normalize_newlines(&endlines);
    TEST(strncmp(endlines.data, endlinesres, endlines.len) == 0, "Carriage return should be replaced with newlines");
}
